...
@app.post("/api/emergency-fix-jobs")
async def emergency_fix_jobs():
    """TEMPORARY: Fix specific jobs that are missing summary.json and pack records"""
    target_jobs = [
        "6706b7a6-142d-4daf-9550-8e4c7f8e7b3d",
        "91a45737-5601-4292-83cc-d61c98e7c3da"
    ]
    
    # Hard-coded user info for the migration (replace with actual user ID)
    user_id = "08192f18-0b1c-4d00-9b90-208c64dd972e"
    r2_directory = f"user_{user_id}"
    ...
    for job_id in target_jobs:
        try:
            print(f"🔧 Processing emergency fix for job {job_id}")
            
            # 1. Fix missing summary.json
            analysis_content = download_from_r2(f"{r2_directory}/{job_id}/analysis_results.json", silent_404=True)
            if analysis_content and not download_from_r2(f"{r2_directory}/{job_id}/summary.json", silent_404=True):
                upload_to_r2(f"{r2_directory}/{job_id}/summary.json", analysis_content)
                results["summary_fixes"].append(job_id)
                print(f"✅ Created summary.json for job {job_id}")
            ...
            if supabase:
                ...
                if not existing_pack.data:
                    ...
                    # Try to get chunk metadata
                    chunks_content = download_from_r2(f"{r2_directory}/{job_id}/chunks_metadata.json", silent_404=True)
                    if chunks_content:
                        try:
                    ...
                    pack_result = supabase.table("packs").insert(pack_data).execute()
                    if pack_result.data:
                        results["pack_fixes"].append(job_id)
